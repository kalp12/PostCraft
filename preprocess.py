...
def process_posts(raw_file_path,processed_file_path = 'data/processed_posts.json',db = True):
    ...
    with open(raw_file_path, 'r', encoding='utf-8') as raw_file:
        ...
        for post in raw_data:
            ...
            metadata = extract_metadata(post['text'].encode('utf-8', errors='ignore').decode('utf-8')) 
...
def extract_metadata(post):
    ...
    1. Return only a valid JSON object. No preamble, no explanation, no markdown, no code fences like ```json.
    ...
    chain = pt | llm | output_parser
    try:
        response = chain.invoke({'post': post})
    except OutputParserException as e:
        raise OutputParserException(
            f"Failed to parse the response: {str(e)}", llm_output=str(e)
        ) from e
    return response
